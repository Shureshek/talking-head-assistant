...
https://github.com/user-attachments/assets/3b8954c0-5f41-412e-85dc-f85366a441f6
...
- В файле client/config.py установите нужное имя в переменной PERSON_NAME.
- В файле docker-compose.yaml укажите это же имя для переменной PERSON_NAME.
### 2. Подготовьте файловую структуру:
- Создайте в директории volumes/audio_samples/ новую папку.
- **Важно:** Имя папки должно строго совпадать со значением PERSON_NAME.
### 3. Добавьте образец голоса:
- Поместите в созданную папку аудиофайл в формате .wav или .ogg.
- Требования к файлу: Длительность от 10 секунд, чистое звучание без фонового шума. Имя файла может быть произвольным.

## Заметки и планы развития
- **Потоковое воспроизведение видео**  
Сейчас MuseTalk генерирует полное видео (mp4) и только потом клиент его воспроизводит.
В планах перейти на настоящий стриминг кадров (frame-by-frame) по сети для минимальной задержки.
- **Ограничение по видеопамяти**  
На текущем этапе стриминг кадров требует больше 24 ГБ VRAM.
Поэтому до его реализации необходимо провести оптимизацию проекта.
